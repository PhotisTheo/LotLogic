...
GISDATA_ROOT = Path(__file__).resolve().parents[2] / "gisdata"
...
def _load_boston_neighborhood_geojson() -> Optional[Dict[str, Any]]:
    if not BOSTON_NEIGHBORHOODS_PATH.exists():
        logger.info("Boston neighborhoods file not found at %s", BOSTON_NEIGHBORHOODS_PATH)
        return None

    try:
        with BOSTON_NEIGHBORHOODS_PATH.open("r", encoding="utf-8") as handle:
...
def _build_massgis_catalog_from_sources(
    *,
    ...
    fallback_catalog = fallback or _load_cached_catalog(enforce_ttl=False)
    if fallback_catalog:
